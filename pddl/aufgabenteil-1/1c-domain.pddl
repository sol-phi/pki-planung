(define (domain sunshine-logistics-1c)
  (:requirements :strips :typing :disjunctive-preconditions :negative-preconditions :numeric-fluents :universal-preconditions)
  ...
  ; Added for 1c. 
  ; Tracks the kilometers driven by the trucks, and the variable that the planner attempts to minimize.
  (:functions
    (kilometers ?t - truck)
    (total-wear)
  )
  ...
  ; 1c: The roads are assumed to have a length of just 1. 
  ; 1c: Since total-wear sums up the kilometer count of the current truck on every trip, 
  ; 1c: the planner minimizing that variable will naturally prefer the truck with the lowest kilometer count, leading to balanced wear. 
  (:action drive-to-location
    ...
    :effect (and (not(truck-at-warehouse ?t ?w_from))
                 ...
                 (increase (kilometers ?t) 1)
                 (increase (total-wear) (kilometers ?t)))
  )
)